...
{
	KX_VertexProxy *self = static_cast<KX_VertexProxy *>(self_v);
	return PyObjectFrom(self->m_vert->co/*self->m_array->GetPosition(self->m_vertexIndex)*/);
}
...
int KX_VertexProxy::pyattr_set_XYZ(EXP_PyObjectPlus *self_v, const struct EXP_PYATTRIBUTE_DEF *attrdef, PyObject *value)
{
	...
	if (PyVecTo(value, vec)) {
		...
		//self->m_array->NotifyUpdate(RAS_DisplayArray::POSITION_MODIFIED);
		copy_v3_v3(self->m_vert->co, vec.data);
		DEG_id_tag_update(&self->m_mesh->id, ID_RECALC_GEOMETRY | ID_RECALC_COPY_ON_WRITE);
...
KX_VertexProxy::KX_VertexProxy(Mesh *mesh, MVert *mvert)
	:m_mesh(mesh),
	m_vert(mvert)
{
}
